...
In the Golden Demo, a buyer requests a fragrance-free sensitive-skin routine under ₹2,000. The agent retrieves real SKU IDs, proposes a moisturizer as a transparent complement, then the policy engine sees the approval threshold and waits. The approval becomes a hash-bound mandate. Only after fresh price and inventory verification can a server create a Razorpay TEST order.

Then we break it on purpose. The price drifts from ₹1,799 to ₹2,049. AgentReady blocks it—with reason codes and no financial action. This is what safe autonomy looks like.
