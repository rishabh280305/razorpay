...
export function useWebMcp(actions: { startGoldenDemo: () => void; runPriceDriftDemo: () => void; navigate: (workspace: string) => boolean }) {
  useEffect(() => {
    ...
    void register({ name: "start_karatsuba_golden_demo", title: "Start Golden Demo", description: "Open the Buyer Agent and start the visible bounded-commerce Golden Demo.", inputSchema: { type: "object", properties: {}, additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute: () => { actions.startGoldenDemo(); return { workspace: "Buyer Agent", started: true }; } });
    void register({ name: "run_karatsuba_price_drift_demo", title: "Run Price Drift Failure", description: "Open the Chaos Lab and visibly demonstrate a price change being blocked before a Razorpay action.", inputSchema: { type: "object", properties: {}, additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute: () => { actions.runPriceDriftDemo(); return { workspace: "Chaos Lab", decision: "DENY", reasonCodes: ["PRICE_CHANGED", "BUDGET_EXCEEDED"], razorpayActionCreated: false }; } });
    void register({ name: "navigate_karatsuba_workspace", title: "Open Karatsuba Workspace", description: "Navigate the visible app to a named Karatsuba workspace.", inputSchema: { type: "object", properties: { workspace: { type: "string", enum: ["Overview", "Buyer Agent", "Catalog", "Agent Readiness", "Sessions", "Approvals", "Policies", "Protocol Lab", "Audit Trail", "Growth Eval", "Chaos Lab", "Developer", "Integrations", "Judge Mode"] } }, required: ["workspace"], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute: (input) => { const workspace = typeof input === "object" && input ? String((input as { workspace?: unknown }).workspace ?? "") : ""; if (!actions.navigate(workspace)) throw new Error("Unknown Karatsuba workspace"); return { workspace, visible: true }; } });
