...
export async function POST(request: NextRequest) {
  ...
  const context = await persistCheckoutContext({ key, requestId, mandate, mandatePayload: mandate, items: parsed.data.items, totalPaise: policy.authoritativeTotalPaise, policyDecision: policy.decision, reasonCodes: policy.reasonCodes, channel: "web" });
