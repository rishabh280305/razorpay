...
export async function POST(request: NextRequest) {
  ...
  let policy;
  try { policy = evaluatePolicyWithCatalog({ items: parsed.data.items, mandate, catalog, proposedTotalPaise: parsed.data.proposedTotalPaise, controls: { baseCartTotalPaise, expectedAgentIdentity: "karatsuba-payment-link", actualAgentIdentity: "karatsuba-payment-link" } }); }
  ...
  const link = await razorpayClient().paymentLink.create({ amount: policy.authoritativeTotalPaise, currency: "INR", accept_partial: false, reference_id: referenceId, description: "Karatsuba policy-authorized cart", customer: { name: "Karatsuba Demo Buyer", email: "buyer@example.com", contact: "+919000000000" }, notify: { email: false, sms: false, whatsapp: false }, reminder_enable: false, notes: { mandate_hash: mandate.canonicalHash } }) as unknown as { id: string; short_url: string; amount: number; status: string };
