...
export async function recordVerifiedPayment(input: { razorpayOrderId: string; razorpayPaymentId: string; state: string }) { const db = getDb(); if (!db) return false; const [updated] = await db.update(commerceOrders).set({ razorpayPaymentId: input.razorpayPaymentId, state: input.state, updatedAt: new Date() }).where(eq(commerceOrders.razorpayOrderId, input.razorpayOrderId)).returning({ id: commerceOrders.id }); return Boolean(updated); }
export async function getRefundEligibleOrder(paymentId: string) { const db = getDb(); if (!db) return null; const [order] = await db.select().from(commerceOrders).where(eq(commerceOrders.razorpayPaymentId, paymentId)).limit(1); return order ?? null; }
export async function markOrderRefunded(paymentId: string) { const db = getDb(); if (!db) return false; const [updated] = await db.update(commerceOrders).set({ state: "REFUNDED", updatedAt: new Date() }).where(eq(commerceOrders.razorpayPaymentId, paymentId)).returning({ id: commerceOrders.id }); return Boolean(updated); }
...
export async function applyWebhookOrderState(razorpayOrderId: string | undefined, event: string, razorpayPaymentId?: string) { const db = getDb(); if (!db || !razorpayOrderId) return; const state = event === "payment.captured" ? "PAYMENT_CAPTURED" : event === "order.paid" ? "ORDER_CONFIRMED" : event === "payment.failed" ? "PAYMENT_FAILED" : event === "payment.authorized" ? "PAYMENT_AUTHORIZED" : null; if (state) await db.update(commerceOrders).set({ state, ...(razorpayPaymentId ? { razorpayPaymentId } : {}), updatedAt: new Date() }).where(eq(commerceOrders.razorpayOrderId, razorpayOrderId)); }
